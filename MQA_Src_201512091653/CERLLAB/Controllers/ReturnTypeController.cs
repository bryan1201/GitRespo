using System.Data;
using System.Linq;
using System.Web.Mvc;
using CERLLAB.Models;
using System.Data.Entity;

namespace CERLLAB.Controllers
{
    public class ReturnTypeController : Controller
    {
        private CERLDBContext db = new CERLDBContext();

        //
        // GET: /ReturnType/

        public ActionResult Index()
        {
            return View(db.ReturnTypes.ToList());
        }

        //
        // GET: /ReturnType/Details/5

        public ActionResult Details(int id = 0)
        {
            ReturnType returntype = db.ReturnTypes.Find(id);
            if (returntype == null)
            {
                return HttpNotFound();
            }
            return View(returntype);
        }

        //
        // GET: /ReturnType/Create

        public ActionResult Create()
        {
            return View();
        }

        //
        // POST: /ReturnType/Create

        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult Create(ReturnType returntype)
        {
            if (ModelState.IsValid)
            {
                db.ReturnTypes.Add(returntype);
                db.SaveChanges();
                return RedirectToAction("Index");
            }

            return View(returntype);
        }

        //
        // GET: /ReturnType/Edit/5

        public ActionResult Edit(int id = 0)
        {
            ReturnType returntype = db.ReturnTypes.Find(id);
            if (returntype == null)
            {
                return HttpNotFound();
            }
            return View(returntype);
        }

        //
        // POST: /ReturnType/Edit/5

        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult Edit(ReturnType returntype)
        {
            if (ModelState.IsValid)
            {
                db.Entry(returntype).State = EntityState.Modified;
                db.SaveChanges();
                return RedirectToAction("Index");
            }
            return View(returntype);
        }

        //
        // GET: /ReturnType/Delete/5

        public ActionResult Delete(int id = 0)
        {
            ReturnType returntype = db.ReturnTypes.Find(id);
            if (returntype == null)
            {
                return HttpNotFound();
            }
            return View(returntype);
        }

        //
        // POST: /ReturnType/Delete/5

        [HttpPost, ActionName("Delete")]
        [ValidateAntiForgeryToken]
        public ActionResult DeleteConfirmed(int id)
        {
            ReturnType returntype = db.ReturnTypes.Find(id);
            db.ReturnTypes.Remove(returntype);
            db.SaveChanges();
            return RedirectToAction("Index");
        }

        protected override void Dispose(bool disposing)
        {
            db.Dispose();
            base.Dispose(disposing);
        }
    }
}