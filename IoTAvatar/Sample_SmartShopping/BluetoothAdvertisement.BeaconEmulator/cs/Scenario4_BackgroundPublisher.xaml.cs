...
namespace BluetoothAdvertisement
{
    ...
    public sealed partial class Scenario4_BackgroundPublisher : Page
    {
        ...
        {
            string customizedMsg = "SetFilter:-30";
            // Unregistering the background task will stop advertising if this is the only client requesting
            // First get the existing tasks to see if we already registered for 

            bool shouldStop = true;
            bool shouldResume = false;

            if (shouldStop && taskRegistration != null)
            {
                taskRegistration.Unregister(true);
                taskRegistration = null;
                shouldResume = true;
            ...
            rootPage.NotifyUser("Change BT-Message: " + customizedMsg, NotifyType.StatusMessage);
            ...
            {
                if (shouldResume)
                    await Task.Delay(500); // wait for BG to stop

                await SendCustomizedMessage(1500, customizedMsg);
