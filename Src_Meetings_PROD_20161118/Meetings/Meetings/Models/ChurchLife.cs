...
namespace Meetings.Models
{
    ...
    public class vMeetingDate
    {
        ...
        public DateTime ENDDT { get; set; }
        [Display(Name = "場次")]
        public IEnumerable<vMeetingDateActivate> vmeetingDateActivate
        {
            get
            {
                return _db.vMeetingDateActivates.Where(x => x.meetingDateId == meetingDateId);
        ...
        public int CountSignMembers(string sex)
        {
            int rslt = 0;
            if (string.IsNullOrEmpty(sex))
                rslt = _db.vMeetingDateMembers.Where(x => x.meetingDateId == this.meetingDateId).Count();
            else
            {
                sex = sex.Trim().ToUpper();
                if (sex == "B" || sex == "S" )
                    rslt = _db.vMeetingDateMembers.Where(x => x.meetingDateId == this.meetingDateId && x.Sex == sex).Count();
                else
                    rslt = _db.vMeetingDateMembers.Where(x => x.meetingDateId == this.meetingDateId).Count();
            }
            return rslt;
    ...
    public class vMeetingDateActivate
    {
        ...
        public vMeetingDateActivate(ChruchLifeDBContext db)
        {
            this._db = db;
        }

        public vMeetingDateActivate()
        {
            if (this._db == null)
                this._db = new ChruchLifeDBContext();
        }

        public int CountSignMembers()
        {
            int rslt = 0;
            
            if (Constant.SexArray.Contains(this.Sex))
                rslt = _db.vMeetingDateActivateMembers.Where(x => x.ActivateId == this.ActivateId && x.Sex == this.Sex).Count();
            else
                rslt = _db.vMeetingDateActivateMembers.Where(x => x.ActivateId == this.ActivateId).Count();
            return rslt;
        }

        public int CountAttend(bool attend)
        {
            int rslt = 0;
            if (Constant.SexArray.Contains(this.Sex))
                rslt = _db.vMeetingDateAttendStatus.Where(x => x.ActivateId == this.ActivateId && x.Sex == this.Sex && x.Attend == attend).Count();
            else
                rslt = _db.vMeetingDateAttendStatus.Where(x => x.ActivateId == this.ActivateId && x.Attend == attend).Count();
            return rslt;
        }
    ...
    public class vMeetingDateMember
    {
        ...
        public vMeetingDateMember GetMeetingMember(Guid meetingdateId, string usercode)
        {
            ...
            Guid uid = _db.UserProfile.Where(x => x.UserCode == usercode).FirstOrDefault().uniqueId;
            if (uid == null)
                return null;
            return _db.vMeetingDateMembers.Where(x => x.UserProfileId == uid && x.meetingDateId == meetingdateId).FirstOrDefault();
        }
    }
    ...
    {//ActivateId, meetingId, ActivateDate, ActivateDesc
        ...
        {
            _db = new ChruchLifeDBContext();
            IEnumerable<vMeetingDateActivate> vmda = _db.vMeetingDateActivates.Where(x => x.meetingDateId == meetingDateId).OrderByDescending(x=>x.ActivateDate);
            return vmda;
        }
    ...
    public class MeetingDateMemberLog
    {
        ...
        private bool ChkLogin(Guid activateid, string usercode)
        {
            ...
            if (uid == null)
                return false;
            vMeetingDateMemberLog vmdml = _db.vMeetingDateMemberLogs.Where(x => x.ActivateId == activateid && x.UserProfileId == uid).FirstOrDefault();
            if (vmdml == null)
                return true;
            else
                return false;
    ...
    public class UserProfile
    {
        ...
        public UserProfile Get(string usercode)
        {
            ...
            if (uid == null)
                return null;
            return _db.UserProfile.Find(uid);
        }
